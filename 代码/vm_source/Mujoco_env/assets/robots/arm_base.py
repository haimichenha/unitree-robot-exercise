import mujoco
import numpy as np
import os,sys
import pathlib
HOME_PATH = str(pathlib.Path("../").parent.resolve())
import sys
sys.path.append(HOME_PATH)
from utils_.KDL_utils import KDL_utils


class ArmBase(object):
    def __init__(self,
                 name=None,
                 urdf_path=None,
                 xml_path=None,
                 gripper=None
                 ):
        self.name = name
        self.urdf_path = urdf_path
        self.xml_path = xml_path
        self.gripper = gripper
        self.robot_model = mujoco.MjModel.from_xml_path(filename=self.xml_path, assets=None)
        self.robot_data = mujoco.MjData(self.robot_model)
        self.arms = []
        # joint PD
        self.kp = None
        self.kd = None

    class Arm(object):
        """
        internal class
        using for precise arm state, double (left and right) or single
        """
        def __init__(self, ArmBase, state, urdf_path):
            self.robot_model = ArmBase.robot_model
            self.robot_data = ArmBase.robot_data
            self.state = state
            self.urdf_path = urdf_path
            self.joint_index = []
            self.gripper_index = []
            self.actuator_index = []
            self.base_joint_index = []
            self.base_actuator_index = []
            self.ee_mocap = None
            self.ee = None
            self.init_pose = None
            self.joint_pose = None
            self.init_base_pose = None
            self.arm_base_link = None
            self.arm_base_pos = None
            self.arm_base_mat = None
            self.name = None
            self.interpolator = None
            
            

        @property 
        def InitKDL(self):
            if self.state == "single":
                self.kdl_solver = KDL_utils(self.urdf_path)
                self.kdl_solver.resetChain(self.arm_base_link,self.ee)
            else:
                print("failed to Init KDL !")

        @property
        def jnt_num(self):
            return len(self.joint_index)
        
        @property
        def gripper_num(self):
            return len(self.gripper_index)

        def getBaseJoint(self):
            """base joints are the rotation joints in robot stand """
            self.base_joint_index = [string for string in self.joint_index if 'base' in string]
            return self.base_joint_index

        def getBaseActuator(self):
            """base actuators are the rotation motors in robot stand """
            self.base_actuator_index = [string for string in self.actuator_index if 'base' in string]
            return self.base_actuator_index

        def setArmInitPose(self, initPose: np.array):
            if len(initPose) == self.jnt_num:
                self.init_pose = initPose
            else:
                raise ValueError("shape error for input pose vector")  # need to optimize
        def setArmJointPose(self, jointPose: np.array):
            if len(jointPose) == self.jnt_num:
                self.joint_pose = jointPose
            else:
                print(len(jointPose),self.jnt_num)
                raise ValueError("shape error for input pose vector")  # need to optimize
            
        def setBaseInitPose(self, initBasePose: np.array):
            if len(self.init_base_pose) == len(initBasePose):
                self.init_base_pose = initBasePose
            else:
                raise ValueError("shape error for input pose vector")  # need to optimize

        def get_Joint_index(self):
            joint_index = []
            for i in range(self.jnt_num):
                joint_index.append(self.joint_index[i])
            return joint_index

        def get_Gripper_index(self):
            '''
            left then right
            '''
            gripper_index = []
            for i in range(len(self.gripper_index)):
                gripper_index.append(self.gripper_index[i])
            return gripper_index

        def get_Joint_id(self):
            joint_id = []
            for i in range(self.jnt_num):
                joint_id.append(mujoco.mj_name2id(self.robot_model, mujoco.mjtObj.mjOBJ_JOINT, self.joint_index[i]))
            return joint_id

        def set_Arm_base_link(self,link):
            self.arm_base_link = link

        def set_Arm_ee_link(self,ee):
            self.ee = ee

        def get_Arm_bias_frame(self):
            xpos = self.robot_data.body(self.arm_base_link).xpos
            xmat = self.robot_data.body(self.arm_base_link).xmat
            return xpos,xmat
            
        def CartPoseComp(self):
            # print("arm_base_link===",self.arm_base_link)
            arm_base_pos, arm_base_mat = self.get_Arm_bias_frame()
            # print("in arm_base:",arm_base_pos)
            self.arm_base_pos = self.kdl_solver.setVec(arm_base_pos)
            self.arm_base_mat = self.kdl_solver.setRot(arm_base_mat).Inverse()
            
            return self.arm_base_pos, self.arm_base_mat

        @property
        def arm_qpos(self):
            qpos_states = []
            for i in range(self.jnt_num):
                qpos_states.append(self.robot_data.joint(self.joint_index[i]).qpos[0])
            return qpos_states

        @arm_qpos.setter
        def arm_qpos(self, value):
            raise ValueError("You cannot set qpos directly.")

        @property
        def arm_qvel(self):
            qvel_states = []
            for i in range(self.jnt_num):
                qvel_states.append(self.robot_data.joint(self.joint_index[i]).qvel[0])
            return qvel_states

        @property
        def arm_torq(self):
            qtorq_states = []
            for i in range(self.jnt_num):
                qtorq_states.append(self.robot_data.actuator(self.actuator_index[i]).ctrl[0])
            return qtorq_states
