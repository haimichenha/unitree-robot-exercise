...
| `代码/` | `vm_source/`、`.zip` 与 `.tar.gz` | 从虚拟机同步的完整实验代码包：ACT、H1 数据采集、MuJoCo/IK、验证及训练监控，共 50 个文件（源码、配置、XML 和说明）；不包含训练数据、检查点或外部项目克隆。 |
| `third_party/RoboJuDo/` | RoboJuDo `release@ed7601f` 快照 | 已明确署名并保留 CC BY 4.0 `LICENSE` 的第三方开源代码，供安装、阅读与研究。 |
...
`代码/来源与边界.md` 明确区分了“本次提交的实验工程代码”和外部依赖：本仓库提交本次 H1/ACT/验证工程以及对 RoboJuDo 的调用与证据适配代码，同时在 `third_party/RoboJuDo/` 保留**已署名、已保留许可证的 RoboJuDo 开源快照**。具体的许可证、作者、提交版本、改动范围与完整性摘要见 `THIRD_PARTY_NOTICES.md`。
...
`CHECKSUMS.sha256` 列出本项目代码、文档、视频和证据的 SHA-256。第三方 RoboJuDo 快照的完整性信息（固定提交、文件数、字节数和聚合树 SHA-256）位于 `THIRD_PARTY_NOTICES.md`。
